 ...
 */
{
    ...
    if(!strncmp(token,"enda",4)) hash=L_endasm;
    else
...
dump_line:	skipnl();			/* Ignore rest of line	*/
	 ...
	 */
    if (counter > 0 && (hash == L_line || hash == L_pragma || hash==L_asm
    || hash==L_endasm)) {
	    putchar('\n');
	    counter--;
	}
...
nest_err:	cerror("#%s must be in an #if", token);
    ...
    case L_asm:
	     ...
	     */
        printf(hash==L_pragma?"#pragma ":(hash==L_endasm?"#endasm ":"#asm "));
	    while ((c = get()) != '\n' && c != EOF_CHAR)
		cput(c);
	    unget();
	    break;
