...
/*
    ...
    if(efmt==1 && digs>prec && digs>1)
    {
        /* round to prec significant figures */
        xp=cp+prec;
        exp+=digs-prec;
        digs=prec;
        if(*xp>='5')
    ...
    if(efmt == 0 && prec == 0 || /* efmt == 1 && exp >= 0 && exp <= 5+ prec */
       efmt == 1 && (exp+digs>=prec || exp>=0) && (exp+digs<=4+prec) ) {
		/* use d format */
...
/*    if(efmt == 1 && (exp >= 5 || exp + digs < -4)) */
    if(efmt == 1 && (exp+digs>prec+5 || exp+digs< -3))
		efmt++;
...
loop:
	...
	while(*cp) {
		...
		if(++digs == exp)
        {
            if(origfmt==1 && !*cp) goto END;
			putc('.');
            afterdecimal=1;
        }
	}
    if(origfmt!=1 || !afterdecimal)
    {
        while(exp > 0) {
            putc('0');
            prec--;
