 ...
 *	CP/M-80 version
 *
 *  C [-T] [-Z] [-C] [-O] [-I] [-F] [-U] [-D] [-S] [-X] [-P] [-W] [-M] files {-Llib}
 */
...
static char	keep,		/* retain .obj files, don't link */
        ...
        wiz,        /* generate code for Sharp Wizards */
...
doit()
{
	...
	rm(RM_FILE, tmpf3);
	if(!keep) {
        flgs[flg_idx++] = wiz?"-Plowpage=8000h,text,data,bss":"-Ptext=0,data,bss";
		if(reloc)
