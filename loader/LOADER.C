...
char cannotopen[]="Can't open!";
char errorwriting[]="Can't write!";
char spaces[]="                  ";
...
{       `
    ...
    ozputch(48/* from sizer */,60,c=mygetch());
	if(c=='0')
	{
		if(numslots!=10)
		  return -1;
...
ozputs0(60,"leaving orphaned data.)");
		if(ozgetch()!=KEY_UPPER_MENU) exit(0);
	}
}

void operating(char *s, char *f)
{
    ozputs(2+ozputs0(0,s),0,f);
}
...
main()
{
	...
	if(modelM)
        ozputs0(21,"R:run  D:delete one  A:delete all");
	else
        ozputs0(21,"S:save  L:load  R:run  D:delete one  A:delete all");
    ozputs0(31,"BCEFGHIJKMNOPQTUVWXYZ-:run in category  ESC:exit");
    while(0xFFFF==(k=mygetch()));
    switch(k)
	{
        case '_':
            category='-';
