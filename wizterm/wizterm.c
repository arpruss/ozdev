...
void domenu(void)
{
    ...
    lowputs("\n"
"Options:\n"
"Change to 300,1200,2400,4800,9600,(a)19200 or (b)38400\n"
"o: Turn off xon/xoff handshaking\n"
"x: Turn on xon/xoff handshaking\n"
"l: Toggle local echo\n"
"h: Toggle display height\n"
"w: Toggle display width\n");
    switch(ozgetch())
    {
        ...
        case 'h':
                  ozcopypage(0,1);
                  if(height==13) setlines16();
                    else setlines13();
                  { extern byte smoothscroll,LNM;
                    byte _LNM=LNM, _ss=smoothscroll; reset_term();
                    LNM=_LNM; smoothscroll=_ss;}
                  return;
        case 'w': setcols(width==59?60:59);
                  break;
...
void do_help()
{
    ...
    xorcursor();
    reverse=1;
    lowputs("Special Wizterm keys:\n");
    reverse=0;
    lowputs("MY PROGRAMS:  quit\n"
            "upper-MENU:   set various options\n"
            "lower-MENU:   this help screen\n"
            "2ND + v:      paste from clipboard\n"
            "2ND + c:      copy screen image to a memo\n"
            "2ND + key:    access purple symbols\n"
            "NEW + key:    CTRL keys\n"
            "CATEGORY  +   one of: opqr" "w190,.\n"
            "              yields: []~\\""`|{}<>\n"
            "CATEGORY + b: send a short break\n"
            "CATEGORY + l: send a long break\n");
    reverse=1;
    lowputs("Press any key to return to Wizterm.");
    reverse=0;
    ozgetch();
    ozcopypage(0,1);
    xorcursor();
 ...
 */
    ...
    if(handshaking)
        ...
        _ozrxhandshaking=0;
    reverse=1;
    termputs(" Welcome to WizTerm 1.0.17 \r\n");
    reverse=0;
    termputs("By Alexander R. Pruss <ap85@georgetown.edu>.\r\n"
             "Press upper MENU key to adjust baud rate,\r\n"
             "lower MENU key for help, and MY PROGRAMS to exit.\r\n");
    while(1)
    {
        ozintwait();
