...
void handleoption(int item, char *o)
{
    ...
    {
        case COLUMNS:
            if(atoi(o)>=60) setcols(60); else setcols(59);
            break;
        case DATABITS:
          switch(atoi(o))
          {
...
void configure(void)
{
    ...
    termputs("Opened zz:wizterm.config...\r\n");
#if 0
    {
    ...
    while(-1!=(c=ozgetc(h)) && c)
    {
        ...
        {
            while(-1!=(c=ozgetc(h)) && c!='\r' && c!='\xff' && c);
            if(!c || c==-1 || c=='\xff') break; else continue;
        }
        while(-1!=(c=ozgetc(h)) && c && isspace(c));
        if(-1==c) break;
        optpos=0;
        do
          theoption[optpos++]=c;
        while((c=ozgetc(h))!=-1 && c!='\r' && c!='\xff' && c);
        theoption[optpos]=0;
